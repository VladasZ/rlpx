...
use crate::{error::Result, remote_ack::RemoteAck};
...
impl Handshake {
    ...
    pub fn encode_auth(&self) -> Result<Vec<u8>> {
        encode_auth(
            &self.remote_pk,
            &self.static_kp,
            &self.ephemeral_kp,
    ...
    pub fn decode_ack(&self, ack: &[u8]) -> Result<RemoteAck> {
...
fn encode_auth(
    ...
    let shared = *ecdh::agree(local_kp.secret(), &remote_pk)?;
    rlp.append(&sign(ephemeral_kp.secret(), &(shared ^ *nonce))?.to_vec());
    ...
    let message = ecies::encrypt(&remote_pk, &prefix, &encoded)?;
...
fn decode_ack(secret: &Secret, data: &[u8]) -> Result<RemoteAck> {
    let ack = ecies::decrypt(secret, &data[0..2], &data[2..])?;
